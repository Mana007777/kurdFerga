<div class="p-6 md:p-10 max-w-6xl mx-auto w-full relative">
    ...
    <div class="relative pt-12 pb-16 mb-16 px-8 text-center group">
        ...
        <div class="absolute top-0 left-1/4 w-64 h-64 bg-violet-500/5 dark:bg-violet-500/10 rounded-full blur-[80px] animate-pulse"></div>
        ...
        <div class="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 z-20">
            <div class="relative w-32 h-32 rounded-full p-1.5 bg-gradient-to-br from-violet-500 via-purple-500 to-pink-500 shadow-[0_0_30px_rgba(99,102,241,0.2)] dark:shadow-[0_0_30px_rgba(99,102,241,0.4)]">
                <div class="w-full h-full rounded-full bg-white dark:bg-[gray-900] p-1.5 overflow-hidden border-2 border-white/10">
                    ...
                    @else
                        <div class="w-full h-full flex items-center justify-center bg-gradient-to-br from-indigo-50 to-slate-100 dark:from-indigo-900 dark:to-slate-900 rounded-full text-plum-600 dark:text-indigo-400">
        ...
        <div class="bg-white/70 dark:bg-[gray-900]/60 backdrop-blur-3xl rounded-[3rem] border border-slate-200 dark:border-white/5 p-12 pt-20 shadow-2xl">
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-violet-500/10 border border-violet-500/20 text-plum-600 dark:text-indigo-400 text-[10px] font-bold uppercase tracking-widest mb-6">
            ...
            <div class="mt-12 flex flex-col items-center gap-4">
                <div class="flex items-center justify-between w-full max-w-md mb-1">
                    ...
                    <span class="text-xs font-bold text-plum-600 dark:text-indigo-400 bg-violet-500/10 px-2 py-0.5 rounded-full">{{ $progressPct }}%</span>
                </div>
        ...
        @forelse($sections as $section)
            <div class="bg-white/40 dark:bg-[gray-900]/40 backdrop-blur-2xl rounded-3xl border border-slate-200 dark:border-white/5 overflow-hidden shadow-sm dark:shadow-xl transition-all duration-300 hover:border-slate-300 dark:hover:border-white/10">
                ...
                <div class="px-8 py-5 bg-slate-50 dark:bg-white/5 border-b border-slate-200 dark:border-white/5 flex items-center gap-4">
                    <div class="w-10 h-10 rounded-xl bg-violet-500/10 border border-violet-500/20 flex items-center justify-center text-plum-600 dark:text-indigo-400">
                        ...
                        <div class="group flex items-center gap-6 px-8 py-5 {{ $isDone ? 'bg-emerald-500/[0.03] dark:bg-emerald-500/5' : 'hover:bg-slate-50 dark:hover:bg-white/5' }} transition-all duration-300">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-3 mb-1">
                                    <h4 @class(['font-bold text-lg leading-tight truncate transition-colors duration-300', 'text-slate-400 line-through decoration-emerald-500/50' => $isDone, 'text-slate-800 dark:text-white group-hover:text-plum-600 dark:group-hover:text-indigo-400' => !$isDone])>
                                        {{ $lesson->title }}
                                    </h4>
                            ...
                            <div class="md:hidden">
                                <flux:icon.chevron-right class="w-5 h-5 text-slate-300 dark:text-gray-600 group-hover:text-plum-600 dark:group-hover:text-violet-500 transition-colors" />
                            </div>
                        </div>
                    @empty
            ...
            @foreach($playlist->courses as $course)
                <div class="bg-white/60 dark:bg-[gray-900]/80 backdrop-blur-3xl rounded-[3rem] p-10 shadow-2xl border border-slate-200 dark:border-white/5 group transition-all duration-500 hover:border-violet-500/20">
                    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-8 mb-10 pb-8 border-b border-slate-100 dark:border-white/5">
                        <div class="max-w-2xl">
                            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-violet-500/10 border border-violet-500/20 text-plum-600 dark:text-indigo-400 text-[10px] font-black uppercase tracking-widest mb-4">
                                Deep Dive Module
                            </div>
                            <h3 class="text-3xl font-black text-slate-900 dark:text-white tracking-tight leading-tight group-hover:text-plum-600 dark:group-hover:text-indigo-400 transition-colors">{{ $course->title }}</h3>
