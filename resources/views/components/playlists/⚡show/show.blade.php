<div class="p-6 md:p-10 max-w-6xl mx-auto w-full relative">
    ...
    <div class="relative pt-12 pb-16 mb-16 px-8 text-center group">
        ...
        <!-- Hero Content -->
        <div class="relative bg-zinc-900/40 backdrop-blur-sm rounded-[3rem] border border-zinc-800 p-12 pt-20 shadow-[-20px_0_80px_-20px_rgba(0,0,0,0.5)]">
            {{-- Instructor Attribution // Top Right --}}
            <div class="absolute top-10 right-10 z-30 group/instructor flex flex-row-reverse items-center gap-4">
                <div class="relative">
                    <div class="w-16 h-16 rounded-full p-1 bg-zinc-900 border border-zinc-800 shadow-xl group-hover/instructor:border-violet-500/50 transition-all duration-500 overflow-hidden text-center">
                        ...
                        @else
                            <div class="w-full h-full rounded-full bg-gradient-to-br from-violet-600 to-indigo-700 flex items-center justify-center text-sm font-black text-white uppercase tracking-tighter shadow-inner">
                ...
                <div class="flex flex-col items-end opacity-0 group-hover/instructor:opacity-100 translate-x-2 group-hover/instructor:translate-x-0 transition-all duration-500 pointer-events-none">
                    <span class="text-xs font-mono text-violet-500 uppercase tracking-[0.2em] leading-none mb-1">{{ __('Instructor') }}</span>
                    <span class="text-base font-black text-white uppercase tracking-widest whitespace-nowrap">{{ $playlist->user->name ?? __('Unknown') }}</span>
                </div>
            </div>
            ...
            @if(!$isEnrolled)
                <!-- Get Started / Enrollment Action -->
                <div class="mt-12 flex flex-col items-center gap-6">
                    <div class="relative group/enroll">
                        <div class="absolute -inset-1 bg-gradient-to-r from-violet-600 to-indigo-600 rounded-3xl blur opacity-30 group-hover/enroll:opacity-60 transition duration-1000 group-hover:duration-200"></div>
                        <flux:button 
                            wire:click="enroll"
                            variant="filled" 
                            class="relative !rounded-2xl !px-16 !py-8 !font-black !text-[13px] !uppercase !tracking-[0.4em] !bg-zinc-950 !border-zinc-800 !text-white hover:!border-violet-500/50 shadow-2xl transition-all duration-300"
                        >
                            {{ __('Get Started') }}
                        </flux:button>
                    </div>
                    
                    <p class="text-[9px] font-black text-zinc-500 uppercase tracking-[0.3em] animate-pulse">
                        {{ __('Join the deployment to access operational files') }}
                    </p>
                </div>
            @else
                <!-- Progress Summary -->
    ...
    @if($isEnrolled)
        ...
        @endif
    @else
        <!-- Enrollment Locked Message -->
        <div class="mt-24 text-center py-20 rounded-[3rem] border border-zinc-800 bg-zinc-900/20 backdrop-blur-sm relative overflow-hidden group">
            ...
            <flux:icon.lock-closed class="w-12 h-12 text-zinc-800 mx-auto mb-6 group-hover:text-violet-500/50 transition-colors duration-700" />
            <h3 class="text-xl font-black text-zinc-500 uppercase tracking-[0.2em]">{{ __('Content Encrypted') }}</h3>
            <p class="text-[10px] font-bold text-zinc-600 uppercase tracking-widest max-w-sm mx-auto mt-2">{{ __('Manual initialization required to reveal technical specifications.') }}</p>
        </div>
    @endif
