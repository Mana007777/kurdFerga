    ...
    <div class="relative z-10 flex flex-col md:flex-row items-center justify-between w-full gap-8">
        ...
        <div class="flex items-center gap-10 w-full md:w-3/4">
            ...
            <div class="flex-shrink-0 relative">
                <div class="w-20 h-20 md:w-24 md:h-24 rounded-2xl bg-zinc-900 border border-zinc-800 flex items-center justify-center text-violet-500 shadow-inner group-hover:border-violet-500/40 transition-all duration-500 relative z-10 overflow-hidden">
            ...
            <div class="flex flex-col space-y-3 flex-1">
                ...
                <div class="flex gap-1 w-full max-w-md pt-2">
                    ...
                    @foreach(range(1, 25) as $i)
                        <div class="h-1.5 flex-1 rounded-full {{ ($i / 25) * 100 <= $progress ? 'bg-violet-500' : 'bg-zinc-900' }} {{ ($i / 25) * 100 <= $progress ? 'shadow-[0_0_8px_rgba(139,92,246,0.5)]' : '' }}"></div>
                    @endforeach
                </div>
            </div>
        </div>
        ...
        <div class="flex-shrink-0 flex flex-col items-end gap-3 w-full md:w-auto">
            <a href="{{ route('playlists.show', $playlist) }}" wire:navigate class="group/btn relative inline-flex items-center gap-4 px-10 py-4 font-black text-white transition-all duration-300 bg-zinc-900 border border-zinc-800 rounded-xl hover:border-violet-500/50 hover:bg-zinc-800 shadow-xl">
...
<div class="relative w-full overflow-hidden rounded-[1.5rem] bg-zinc-950 border border-zinc-800 p-12 flex items-center justify-center">
    <div class="text-center space-y-6">
        <div class="w-16 h-16 bg-zinc-900 border border-zinc-800 rounded-2xl flex items-center justify-center mx-auto text-zinc-700 shadow-inner">
